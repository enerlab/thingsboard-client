...
Requirements: `NPM_TOKEN` repo secret must be an npm **Granular Access Token with "Bypass 2FA" enabled** (the modern equivalent of the legacy Automation token; a token without Bypass 2FA fails with `EOTP` under the org's 2FA-on-write policy). To retry a failed publish, bump the `<client-revision>` segment and merge again, or use the workflow's `workflow_dispatch` button.
